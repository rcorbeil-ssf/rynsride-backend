#!/bin/bash
# this test script should NOT find a match, loc is 2.9 miles away, and pickupRadius is 2 miles
clear
...
curl -X POST --header "Content-Type: application/json" --header "Accept: application/json" -d '{"postedTrip":{  "driverId": "1",
  ...
  "state": "new",
  "id": "1"  }}' $URL"PostedTrips/postAndSearch"
  
# Request a ride
echo "Request a ride"
curl -X POST --header "Content-Type: application/json" --header "Accept: application/json" -d '{"requestedRide":{
  "riderId": "2",
  "startAddress": { "street": "2500 Elm Ave", "city": "San Diego", "state": "CA", "zip": "92154"},
  "startGeopoint": {
    "lat": 32.581027,
    "lng": -117.079774
  },
