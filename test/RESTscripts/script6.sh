#!/bin/bash
#  this test script should NOT find a match, if both rides start more then 31 minutes apart, it's a no go, this ride is 40 minutes apart.
clear
...
curl -X POST --header "Content-Type: application/json" --header "Accept: application/json" -d '{"requestedRide":{
  ...
  "beenRated": false,
  "state": "new",
  "createDate": "2016-04-08"}}' $URL"RideRequests/requestRideAndSearch"
